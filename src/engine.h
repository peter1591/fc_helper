...
namespace engine_detail {
	...
	void performUpgradeAmount(State& state) {
		...
		state.upgrade_amount.cost = state.upgrade_amount.next_cost_multipler * state.upgrade_amount.cost;
		state.income.amount += state.upgrade_amount.income_increase;

		if (state.upgrade_amount.multiply.upgrades <= 0) {
			state.income.amount = state.upgrade_amount.multiply.multiply * state.income.amount;
	...
	bool performUpgradeTime(State& state) {
		...
		state.upgrade_time.cost = state.upgrade_time.next_cost_multipler * state.upgrade_time.cost;
		return true;
	}
}
